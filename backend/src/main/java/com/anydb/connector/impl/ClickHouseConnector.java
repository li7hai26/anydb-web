...
public class ClickHouseConnector implements DatabaseConnector {
    ...
    public boolean testConnection(DatabaseConfig config) {
        ...
        try {
            log.debug("测试ClickHouse连接: {}:{}", config.getHost(), config.getPort());
            ...
            log.warn("ClickHouse连接测试异常: {}", e.getMessage());
            return false;
        } finally {
    ...
    public QueryResult executeQuery(DatabaseConfig config, String sql) {
        ...
        try {
            log.debug("执行ClickHouse SQL查询: {}", sql);
            ...
            log.error("ClickHouse SQL执行失败: {}", e.getMessage());
            throw new DatabaseOperationException(
                "SQL_EXECUTION_FAILED",
                "ClickHouse SQL执行失败: " + e.getMessage(),
                "EXECUTE_QUERY",
                e
            );
        } catch (Exception e) {
            log.error("ClickHouse SQL执行异常: {}", e.getMessage());
            throw new DatabaseOperationException(
                "INTERNAL_ERROR",
                "ClickHouse SQL执行异常: " + e.getMessage(),
                "EXECUTE_QUERY",
                e
            );
        } finally {
    ...
    public UpdateResult executeUpdate(DatabaseConfig config, String sql) {
        ...
        try {
            log.debug("执行ClickHouse SQL更新: {}", sql);
            ...
            log.debug("ClickHouse更新操作完成，影响行数: {}", affectedRows);
            return result;
            
        } catch (SQLException e) {
            log.error("ClickHouse SQL更新失败: {}", e.getMessage());
            throw new DatabaseOperationException(
                "SQL_EXECUTION_FAILED",
                "ClickHouse SQL更新失败: " + e.getMessage(),
                "EXECUTE_UPDATE",
                e
            );
        } catch (Exception e) {
            log.error("ClickHouse SQL更新异常: {}", e.getMessage());
            throw new DatabaseOperationException(
                "INTERNAL_ERROR",
                "ClickHouse SQL更新异常: " + e.getMessage(),
                "EXECUTE_UPDATE",
                e
            );
        } finally {
    ...
    public List<String> getDatabases(DatabaseConfig config) {
        ...
        try {
            ...
            log.error("获取ClickHouse数据库列表失败: {}", e.getMessage());
            throw new DatabaseOperationException(
                "QUERY_FAILED",
                "ClickHouse获取数据库列表失败: " + e.getMessage(),
                "GET_DATABASES",
                e
            );
        } catch (Exception e) {
            log.error("获取ClickHouse数据库列表异常: {}", e.getMessage());
            throw new DatabaseOperationException(
                "INTERNAL_ERROR",
                "ClickHouse获取数据库列表异常: " + e.getMessage(),
                "GET_DATABASES",
                e
            );
        } finally {
    ...
    public List<TableInfo> getTables(DatabaseConfig config, String database) {
        ...
        try {
            log.debug("获取ClickHouse表列表，数据库: {}", database);
            ...
            log.error("获取ClickHouse表列表失败: {}", e.getMessage());
            throw new DatabaseOperationException(
                "QUERY_FAILED",
                "ClickHouse获取表列表失败: " + e.getMessage(),
                "GET_TABLES",
                e
            );
        } catch (Exception e) {
            log.error("获取ClickHouse表列表异常: {}", e.getMessage());
            throw new DatabaseOperationException(
                "INTERNAL_ERROR",
                "ClickHouse获取表列表异常: " + e.getMessage(),
                "GET_TABLES",
                e
            );
        } finally {
    ...
    public TableInfo getTableInfo(DatabaseConfig config, String database, String tableName) {
        ...
        try {
            log.debug("获取ClickHouse表信息，数据库: {}, 表: {}", database, tableName);
            ...
            log.error("获取ClickHouse表信息失败: {}", e.getMessage());
            throw new DatabaseOperationException(
                "QUERY_FAILED",
                "ClickHouse获取表信息失败: " + e.getMessage(),
                "GET_TABLE_INFO",
                e
            );
        } catch (Exception e) {
            log.error("获取ClickHouse表信息异常: {}", e.getMessage());
            throw new DatabaseOperationException(
                "INTERNAL_ERROR",
                "ClickHouse获取表信息异常: " + e.getMessage(),
                "GET_TABLE_INFO",
                e
            );
        } finally {
    ...
    public QueryResult getTableData(DatabaseConfig config, String database, String tableName, 
        ...
        try {
            log.debug("获取ClickHouse表数据，数据库: {}, 表: {}, 页码: {}, 每页: {}", 
            ...
            log.error("获取ClickHouse表数据失败: {}", e.getMessage());
            throw new DatabaseOperationException(
                "QUERY_FAILED",
                "ClickHouse获取表数据失败: " + e.getMessage(),
                "GET_TABLE_DATA",
                e
            );
        } catch (Exception e) {
            log.error("获取ClickHouse表数据异常: {}", e.getMessage());
            throw new DatabaseOperationException(
                "INTERNAL_ERROR",
                "ClickHouse获取表数据异常: " + e.getMessage(),
                "GET_TABLE_DATA",
                e
            );
        } finally {
    ...
    public Object createConnectionPool(DatabaseConfig config) {
        ...
        try {
            log.info("创建ClickHouse连接池，配置: {}:{}", config.getHost(), config.getPort());
            
            if (testConnection(config)) {
                log.info("ClickHouse连接池创建成功");
                return new ClickHouseConnectionPool(config);
            } else {
                throw new DatabaseOperationException(
                    "CONNECTION_FAILED",
                    "ClickHouse连接池创建失败：连接测试失败"
                );
            }
        } catch (Exception e) {
            log.error("创建ClickHouse连接池失败: {}", e.getMessage());
            throw new DatabaseOperationException(
                "CONNECTION_FAILED",
                "ClickHouse连接池创建失败: " + e.getMessage(),
    ...
    @Override
    public void close() {
        log.debug("关闭ClickHouse连接器");
        // ClickHouse连接器在Spring容器关闭时会自动清理资源
    ...
    private QueryResult executeClickHouseSpecialQuery(Statement stmt, String sql) throws SQLException {
        ...
        try {
            ...
            throw new DatabaseOperationException(
                "SQL_EXECUTION_FAILED",
                "ClickHouse特殊查询执行失败: " + e.getMessage(),
                "SPECIAL_QUERY",
                e
            );
        }
