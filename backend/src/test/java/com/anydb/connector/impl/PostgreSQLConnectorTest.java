...
class PostgreSQLConnectorTest {
    ...
    void setUp() {
        ...
        // 设置URL构建
        when(mockConfig.buildUrl()).thenReturn("jdbc:postgresql://" + TEST_HOST + ":" + TEST_PORT + "/" + TEST_DATABASE);
    }
    ...
    void testGetSupportedType() {
        DatabaseType result = postgresqlConnector.getSupportedType();
        
        assertEquals(DatabaseType.POSTGRESQL, result, "应该返回PostgreSQL数据库类型");
        assertEquals("postgresql", result.getCode(), "数据库类型代码应该正确");
        assertEquals("PostgreSQL", result.getDisplayName(), "数据库显示名称应该正确");
    }
    ...
    void testConnection_Success() throws SQLException {
        ...
        boolean result = postgresqlConnector.testConnection(mockConfig);
    ...
    void testExecuteQuery_SimpleQuery_Success() throws SQLException {
        ...
        var result = postgresqlConnector.executeQuery(mockConfig, "SELECT id, name, created_at FROM test_table LIMIT 10");
    ...
    @Test
    @DisplayName("执行复杂查询 - PostgreSQL特有查询")
    void testExecuteQuery_ComplexQuery_Success() throws SQLException {
        // 模拟执行复杂查询
        when(DriverManager.getConnection(anyString(), any(Properties.class))).thenReturn(mockConnection);
        when(mockConnection.createStatement()).thenReturn(mockStatement);
        when(mockStatement.executeQuery(contains("pg_"))).thenReturn(mockResultSet);
        when(mockResultSet.getMetaData()).thenReturn(mockMetaData);
        
        when(mockMetaData.getColumnCount()).thenReturn(2);
        when(mockMetaData.getColumnLabel(1)).thenReturn("schemaname");
        when(mockMetaData.getColumnLabel(2)).thenReturn("tablename");
        
        when(mockResultSet.next()).thenReturn(true, true, false);
        when(mockResultSet.getObject(1)).thenReturn("public", "public");
        when(mockResultSet.getObject(2)).thenReturn("users", "orders");
        
        var result = postgresqlConnector.executeQuery(mockConfig, "SELECT schemaname, tablename FROM pg_tables WHERE schemaname = 'public'");
        
        assertNotNull(result, "查询结果不应该为空");
        assertTrue(result.getRows().size() >= 1, "应该返回表信息");
    ...
    void testExecuteQuery_SQLSyntaxError() throws SQLException {
        ...
        assertThrows(DatabaseOperationException.class, () -> {
            postgresqlConnector.executeQuery(mockConfig, "SELECT * FROM invalid_syntax");
        }, "应该抛出数据库操作异常");
    ...
    void testExecuteUpdate_Success() throws SQLException {
        ...
        var result = postgresqlConnector.executeUpdate(mockConfig, "INSERT INTO test VALUES (1, 'test')");
    ...
    void testExecuteUpdate_Failure() throws SQLException {
        ...
        assertThrows(DatabaseOperationException.class, () -> {
            postgresqlConnector.executeUpdate(mockConfig, "INSERT INTO invalid VALUES (1, 'test')");
        }, "应该抛出数据库操作异常");
    ...
    void testGetDatabases_Success() throws SQLException {
        when(DriverManager.getConnection(anyString(), any(Properties.class))).thenReturn(mockConnection);
        when(mockConnection.createStatement()).thenReturn(mockStatement);
        when(mockStatement.executeQuery("SELECT datname FROM pg_database WHERE datistemplate = false")).thenReturn(mockResultSet);
        
        when(mockResultSet.next()).thenReturn(true, true, true, false);
        when(mockResultSet.getString(1)).thenReturn("postgres", "test_db", "mydb");
        
        List<String> databases = postgresqlConnector.getDatabases(mockConfig);
        
        assertNotNull(databases, "数据库列表不应该为空");
        assertEquals(3, databases.size(), "数据库数量应该正确");
        assertTrue(databases.contains("postgres"), "应该包含postgres数据库");
        assertTrue(databases.contains("test_db"), "应该包含test_db数据库");
        assertTrue(databases.contains("mydb"), "应该包含mydb数据库");
    ...
    void testGetDatabases_Failure() throws SQLException {
        when(DriverManager.getConnection(anyString(), any(Properties.class))).thenReturn(mockConnection);
        when(mockConnection.createStatement()).thenReturn(mockStatement);
        when(mockStatement.executeQuery("SELECT datname FROM pg_database WHERE datistemplate = false")).thenThrow(new SQLException("权限不足"));
        
        assertThrows(DatabaseOperationException.class, () -> {
            postgresqlConnector.getDatabases(mockConfig);
        }, "应该抛出数据库操作异常");
    ...
    void testGetTables_Success() throws SQLException {
        when(DriverManager.getConnection(anyString(), any(Properties.class))).thenReturn(mockConnection);
        when(mockConnection.createStatement()).thenReturn(mockStatement);
        when(mockStatement.executeQuery("SELECT tablename FROM pg_tables WHERE schemaname = 'public'")).thenReturn(mockResultSet);
        
        when(mockResultSet.next()).thenReturn(true, true, false);
        when(mockResultSet.getString(1)).thenReturn("users", "orders");
        
        List<TableInfo> tables = postgresqlConnector.getTables(mockConfig, TEST_DATABASE);
    ...
    void testGetTables_WithDatabase_Success() throws SQLException {
        when(DriverManager.getConnection(anyString(), any(Properties.class))).thenReturn(mockConnection);
        when(mockConnection.createStatement()).thenReturn(mockStatement);
        when(mockStatement.executeQuery("SELECT tablename FROM pg_tables WHERE schemaname = 'test_db'")).thenReturn(mockResultSet);
        
        when(mockResultSet.next()).thenReturn(true, false);
        when(mockResultSet.getString(1)).thenReturn("test_table");
        
        List<TableInfo> tables = postgresqlConnector.getTables(mockConfig, "test_db");
        ...
        // 验证使用了指定的数据库名称
        verify(mockStatement, times(1)).executeQuery("SELECT tablename FROM pg_tables WHERE schemaname = 'test_db'");
    }
    ...
    void testGetTableInfo_Success() throws SQLException {
        when(DriverManager.getConnection(anyString(), any(Properties.class))).thenReturn(mockConnection);
        when(mockConnection.createStatement()).thenReturn(mockStatement);
        when(mockStatement.executeQuery(contains("information_schema.columns"))).thenReturn(mockResultSet);
        
        when(mockResultSet.next()).thenReturn(true, true, true, false);
        when(mockResultSet.getString("column_name")).thenReturn("id", "name", "created_at");
        when(mockResultSet.getString("data_type")).thenReturn("integer", "character varying", "timestamp without time zone");
        when(mockResultSet.getString("is_nullable")).thenReturn("NO", "YES", "NO");
        when(mockResultSet.getString("column_default")).thenReturn("nextval('test_id_seq'::regclass)", "NULL", "now()");
        when(mockResultSet.getString("column_comment")).thenReturn("主键", "名称", "创建时间");
        
        TableInfo table = postgresqlConnector.getTableInfo(mockConfig, TEST_DATABASE, TEST_TABLE);
        ...
        assertEquals("integer", firstColumn.getType(), "第一列类型应该正确");
        assertEquals("NO", firstColumn.getNullable() ? "YES" : "NO", "第一列可空性应该正确");
        assertEquals("主键", firstColumn.getComment(), "第一列注释应该正确");
        assertTrue(firstColumn.isPrimaryKey(), "第一列应该被标记为主键");
    ...
    void testGetTableData_Pagination_Success() throws SQLException {
        ...
        var result = postgresqlConnector.getTableData(mockConfig, TEST_DATABASE, TEST_TABLE, 2, 20, "id", "ASC");
    ...
    void testGetTableData_FirstPage_Success() throws SQLException {
        ...
        var result = postgresqlConnector.getTableData(mockConfig, TEST_DATABASE, TEST_TABLE, 1, 10, null, null);
    ...
    void testExceptionHandling_DatabaseOperationException() throws SQLException {
        ...
        assertThrows(DatabaseOperationException.class, () -> {
            postgresqlConnector.executeQuery(mockConfig, "SELECT 1");
        }, "应该抛出数据库操作异常");
        ...
        try {
            ...
            assertTrue(e.getMessage().contains("PostgreSQL SQL执行失败"), "异常信息应该包含PostgreSQL SQL执行失败");
        }
    }
    ...
    void testSQLInjectionProtection() throws SQLException {
        ...
        var result = postgresqlConnector.executeQuery(mockConfig, maliciousSql);
        
        assertNotNull(result, "应该正常执行查询");
        // PostgreSQL连接器使用原生JDBC，SQL注入防护主要由应用层和数据库本身完成
    ...
    void testExecutionTimeRecording() throws SQLException {
        ...
        var result = postgresqlConnector.executeQuery(mockConfig, "SELECT 1");
    ...
    void testCreateConnectionPool_Success() throws SQLException {
        ...
        var pool = postgresqlConnector.createConnectionPool(mockConfig);
        
        assertNotNull(pool, "连接池不应该为空");
        assertTrue(pool instanceof PostgreSQLConnector.PostgreSQLConnectionPool, "应该返回PostgreSQL连接池实例");
    ...
    void testCreateConnectionPool_Failure() throws SQLException {
        ...
        assertThrows(RuntimeException.class, () -> {
            postgresqlConnector.createConnectionPool(mockConfig);
        }, "应该抛出运行时异常");
    }
    ...
    @Test
    @DisplayName("PostgreSQL特有查询测试")
    void testPostgreSQLSpecialQueries() throws SQLException {
        when(DriverManager.getConnection(anyString(), any(Properties.class))).thenReturn(mockConnection);
        when(mockConnection.createStatement()).thenReturn(mockStatement);
        ...
        when(mockMetaData.getColumnCount()).thenReturn(1);
        when(mockMetaData.getColumnLabel(1)).thenReturn("version");
        when(mockResultSet.next()).thenReturn(true, false);
        when(mockResultSet.getObject(1)).thenReturn("PostgreSQL 15.0");
        
        var result = postgresqlConnector.executeQuery(mockConfig, "SELECT version()");
        
        assertNotNull(result, "特殊查询结果不应该为空");
        assertTrue(result.getRows().size() >= 1, "应该返回版本信息");
    }
    
    @Test
    @DisplayName("PostgreSQL系统表查询测试")
    void testPostgreSQLSystemTableQuery() throws SQLException {
        when(DriverManager.getConnection(anyString(), any(Properties.class))).thenReturn(mockConnection);
        when(mockConnection.createStatement()).thenReturn(mockStatement);
        when(mockStatement.executeQuery("SELECT schemaname, tablename FROM pg_tables WHERE schemaname NOT IN ('information_schema', 'pg_catalog')")).thenReturn(mockResultSet);
        when(mockResultSet.getMetaData()).thenReturn(mockMetaData);
        ...
        when(mockMetaData.getColumnLabel(2)).thenReturn("tablename");
        when(mockResultSet.next()).thenReturn(true, false);
        when(mockResultSet.getObject(1)).thenReturn("public");
        when(mockResultSet.getObject(2)).thenReturn("users");
        
        var result = postgresqlConnector.executeQuery(mockConfig, "SELECT schemaname, tablename FROM pg_tables WHERE schemaname NOT IN ('information_schema', 'pg_catalog')");
        
        assertNotNull(result, "系统表查询结果不应该为空");
        assertTrue(result.getRows().size() >= 1, "应该返回用户表信息");
    }
    ...
    void testEmptyConfiguration() {
        assertThrows(IllegalArgumentException.class, () -> {
            postgresqlConnector.testConnection(null);
        }, "空配置应该抛出异常");
    }
    ...
    void testEmptySQL() throws SQLException {
        ...
        // 空SQL应该正常执行，但返回空结果集
        var result = postgresqlConnector.executeQuery(mockConfig, "");
    ...
    void testConcurrentQueries() throws SQLException, InterruptedException {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            threads[i] = new Thread(() -> {
                results[index] = postgresqlConnector.executeQuery(mockConfig, "SELECT 1");
            });
            threads[i].start();
        }
